...
In this thesis, the ISO 25010 model is adopted since it is the recent quality standard. Additionally, understandability is considered a part of the maintainability, in alignment with many researchers in the field \cite{xia2017measuring}. 
...
\begin{tikzpicture}[node distance=2.5cm, every node/.style={font=\small}]
  ...
  % Arrow from overlap area to Code Quality
  \draw[->, thick] ($(readability.south)!0.5!(comprehension.south)$) -- (quality.north);
\end{tikzpicture}
...
Figure 2.4 presents Java code snippet, which gives an output the words “Hello, world”  \cite{hanenberg2024indentation}.
...
% Set all nodes to have the same width and height
...
The length of the comments can influence completeness of the comments \cite{steidl2013quality}. Shorter comments present less information than longer comments. Figure 2.9 presents a longer code comment, giving more information about the code snippet, while Figure 2.10 visualizes a very short comment.
...
\begin{table}[ht]
\centering
\small % You can use \footnotesize or \scriptsize for even smaller fonts
...
\node (eyeTracking) [box] {Eye-Tracking  data \\[5mm] $\rightarrow$ Measures gaze \\ $\rightarrow$ Measures cognitive load};
...
\begin{figure} [H]
  \centering
  \includegraphics[scale=0.9]{figures/codeb.png}
...
\begin{figure} [H]
  \centering
  \includegraphics[scale=0.3]{figures/setup.png}
...
\hline
Comment quality score & This variable was derived from Question 4: "How do you assess the commenting of the code?" Participants rate the code on a Likert scale, where 1 = "Confusing" and 5  = "Very helpful". \\
\hline
Indentation quality score & This variable was derived from Question 5: "How do you assess the indentation of the code?" Participants rate the indentation of the code on a Likert scale, where 1 = "Unreadable" and 5 = "Very easy to read". \\
\hline
...
\hline
Confidence rating & This variable was derived from Question 2: “How confident are you in your answer to the first question?” (the first question is related to the function of the corresponding code snippet).  Participants rate their code confidence on a Likert scale, where 1 = "Not confident" and  5 = "Very confident".\\
\hline
...
Figure ... shows the design of the first part of questionnaire. The first part of the questionnaire gives general information about the participants. 
...
Participants answer five questions corresponding to each code snippet. These questions are related to the code snippet’s function, ratings of the code comprehension, code indentation, and code commenting. Figure ... shows the design of the main part of questionnaire.
...
\begin{figure} [H]
  \centering
  \includegraphics[scale=0.1]{figures/user.jpg}
...
\hline
\textbf{Variable} & \textbf{Hypothesis (H0)} & \textbf{H0 Rejected?} \\
\hline
Indentation quality score & 4-space $\leq$ No indentation & Rejected \\
\hline
Indentation quality score & 4-space $\leq$ Random indentation & Rejected \\
\hline
Indentation quality score & Random $\leq$ No indentation & Rejected \\
\hline
Comprehension score & Random = No indentation & Not Rejected \\
\hline
Confidence rating & Random = No indentation & Not Rejected \\
\hline
\end{tabular}
...
Figure .... shows the medians for both comment formats.
...
The test produced a p-value of  0.01059, which is below the significance level $\alpha = 0.05$. Therefore, the null hypothesis (H0) was rejected.  This result suggests that the Confidence rating  for the code with redundant comments was significantly higher than that for the code without comments.   Figure ... presents an overview of all hypotheses related to the comment formats.
...
\textbf{Variable} & \textbf{Hypothesis (H0)} & \textbf{H0 Rejected?} \\
\hline
Comment quality score & Helpful $\leq$ No comments & Rejected \\
\hline
Comment quality score& Helpful $\leq$ Minimal comments & Rejected \\
\hline
Comment quality score & Helpful $\leq$ Redundant comments & Rejected \\
\hline
Comment quality score & Minimal = Redundant comments & Not Rejected \\
\hline
Comprehension score & Helpful $\leq$ No comments & Rejected \\
\hline
Comprehension score & Helpful $\leq$ Minimal comments & Rejected \\
\hline
Comprehension score & Redundant $\leq$ Minimal comments & Rejected \\
\hline
Confidence rating & Helpful $\leq$ No comments & Rejected \\
\hline
Confidence rating & Helpful $\leq$ Minimal comments & Rejected \\
\hline
Confidence rating & Redundant $\leq$ Minimal comments & Rejected \\
\hline
Confidence rating & Redundant $\leq$ No comments & Rejected \\
\hline
\end{tabular}
...
In all three code snippets with different levels of indentation, a single AOI category was defined: keywords, including terms such as “if” and “else”. In the code snippet without indentation, participants tended to look at these keywords and often returned to previous lines. Moreover, they showed long fixation durations.  Figure … presents a scanpath from one participant who had spent a lot of time looking at the code and had returned to previous lines. In the code snippet with 4-space indentation, participants tended to follow a clear reading path. Moreover, fixation durations on keywords were notably shorter. Figure … presents a scanpath for one participant who looked at the code lines from the top to bottom without returning to previous lines. In the code snippet with random indentation, participants often had a large amount of fixation duration on the keywords.  Moreover, they often read the code with many jumps between lines and with inconsistent gaze directions. For instance, the Figure  … presents a scanpath for one participant that has followed a “zig-zag” pattern — shifting his eyes inconsistently from left to right and back again.   
